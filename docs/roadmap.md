# 🗺️ SP Analyzer & Validator 프로젝트 로드맵

본 문서는 기존 데이터베이스(SQL Server)에 있는 복잡한 비즈니스 로직을 분석하여, 안전하게 새로운 현대식 프로그램(C#, Java 등)으로 변환하고 그 결과가 정확한지 검증하는 **전체 마이그레이션 과정과 일정**을 쉽게 설명한 로드맵입니다.
...
flowchart LR
    ...
    subgraph Stage1 ["1단계. 분석 및 문서화"]
        direction TB
        DB로그인["1. DB 접속"] --> DFS의존성추적["2. 연관된 표/코드 자동 수집"]
    ...
    subgraph Stage3 ["3단계. 정밀 비교 검증"]
        ...
        데이터1대1비교대조 --> 정합성보고서도출_CompareReport["8. 검증 결과 보고서 발급 (*_CompareReport.md)"]
    end
...
| 마일스톤 단계 | 비즈니스 실행 흐름 | 사용자가 넣는 것 (Input) | 시스템이 주는 것 (Output) | 품질 보증 약속 (Quality) |
| :--- | :--- | :--- | :--- | :--- |
| **Milestone 1** | **레거시 의존성 수집 및 역공학 흐름** | 대상 데이터베이스 접속 및 분석할 프로그램 선택 | 기존 코드 구조와 프로그램 상세 내용 화면 표시 | 조회 권한이 없더라도 중단되지 않고 안전하게 다음 단계를 이어감 (**안전 조치**) |
| **Milestone 2** | **설계 명세서 신뢰성 검증 흐름** | AI가 분석한 최초 설명서 | 문법 검사 및 도표가 포함된 설명서 파일 저장 | 사람이 직접 피드백을 주어 설명서의 신뢰성을 끝까지 확보함 (**3중 검증**) |
| **Milestone 3** | **비용 통제 및 캐시 건너뛰기 흐름** | 재분석 요청 | 변경되지 않은 프로그램은 분석을 건너뛰고 빠른 화면 표시 | 이미 분석한 내용은 호출하지 않아 AI 사용 요금을 획득 절감함 (**비용 통제**) |
| **Milestone 4** | **터미널 상속형 코드 자동 생성 흐름** | 최종 승인된 이전 계획 | AI가 자동으로 작성한 최신 언어의 프로그램 코드 | AI와 대화하며 프로그램을 즉석에서 수정 및 완성 (**단절 없는 구현**) |
| **Milestone 5** | **구현 소스코드 일치성 추적 흐름** | 자동 완성된 프로그램 코드 폴더 | 설명서와 프로그램 소스코드 간의 불일치 보고서 | 설계와 다르게 코딩된 부분이 없는지 확인 (**설계-구현 일치**) |
| **Milestone 6** | **격리 샌드박스 기반 정합성 대조 흐름** | 테스트할 조건 및 데이터 | 이전 및 이후 프로그램의 실행 결과 1:1 비교 보고서 | 날짜 형식이나 소수점 차이 등 미세한 표현 오류를 걸러내고 정밀한 수치 일치 확인 (**결과 정합성**) |
...
*   **쉬운 작동 흐름**:
    1.  **1차 기계 검사**: AI가 답변을 작성하면, 문서 서식이 맞는지와 도표에 문법 오류가 없는지 컴퓨터가 먼저 꼼꼼하게 검사합니다. 오류가 발견되면 AI에게 즉시 다시 쓰도록 지시합니다.
    2.  **2차 AI 상호 검토**: 통과된 문서를 다른 종류의 AI 검토관이 읽으며 번역 오류나 업무적 결함이 없는지 교차로 확인하고 스스로 교정합니다.
    3.  **3차 사람 최종 확인**: 자동 검증이 끝난 후 사용자가 화면에서 내용을 미리 보고 필요시 피드백을 전달해 수정합니다. 사용자가 만족하여 "승인"을 누르면 문서가 최종 저장됩니다.
...
*   **쉬운 작동 흐름**:
    ...
    3.  **망 분리(폐쇄망) 대응**: 인터넷이 되는 개발 환경에서 비용을 아끼며 설명서 작성과 프로그램 생성을 100% 진행하고, 인터넷이 차단된 안전한 내부망 환경에서는 LLM 호출 없이 오직 실행 정합성만을 검증하도록 시스템 흐름을 통제합니다.
...
*   **쉬운 작동 흐름**:
    ...
    3.  AI 코딩 엔진(Claude CLI 등)이 자동으로 켜지면서 작성해야 하는 코드를 인식합니다.
