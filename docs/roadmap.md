...
flowchart LR
    ...
    subgraph Stage1 ["1. 역공학 및 분석 단계"]
        direction TB
        DB로그인["DB 로그인"] --> DFS의존성추적["DFS 의존성 추적 (스키마/주석 수집)"]
        DFS의존성추적 --> AI분석및교차리뷰["AI 분석 및 교차 리뷰"]
        AI분석및교차리뷰 --> 3단계검증루프["3단계 검증 루프 (L1-L2-L3 검증)"]
        3단계검증루프 --> 명세서저장_Spec["명세서 저장 (*_Spec.md)"]
    end
    
    subgraph Stage2 ["2. 배치 현대화 및 코드 생성"]
        direction TB
        TUI순차선택["TUI 순차 선택"] --> 배치전환계획서작성["배치 전환 계획서 작성 (Job 단위 묶음)"]
        배치전환계획서작성 --> 통합지시서번들링["통합 지시서 번들링"]
        통합지시서번들링 --> 코딩에이전트자동기동["코딩 에이전트 자동 기동 (콘솔 스트림 상속)"]
        코딩에이전트자동기동 --> 타겟소스코드생성["타겟 소스코드 생성 (C#/Java 구현 완료)"]
    end
    
    subgraph Stage3 ["3. 정합성 검증 및 대조"]
        direction TB
        스마트파일매핑["스마트 파일 매핑"] --> AI일치성Gap분석["AI 일치성 Gap 분석 (설계 vs 소스 불일치 감지)"]
        AI일치성Gap분석 --> 테스트케이스_및_모의데이터생성["테스트 케이스 및 모의 데이터 생성"]
        테스트케이스_및_모의데이터생성 --> Sandbox_DB적재["Sandbox DB 적재 (Seeding)"]
        Sandbox_DB적재 --> 양방향결과덤프수집["양방향 결과 덤프 수집 (Legacy vs Target 구동)"]
        양방향결과덤프수집 --> Sandbox_DB소거["Sandbox DB 소거 (Cleanup: 격리 상태 원복)"]
        Sandbox_DB소거 --> 데이터1대1비교대조["데이터 1:1 비교 대조 (실수/날짜 정규화 비교)"]
        데이터1대1비교대조 --> 정합성보고서도출_CompareReport["정합성 보고서 도출 (*_CompareReport.md)"]
    end

    %% 메인 흐름: Stage들을 가로로 연결
    Start([시작]) --> Stage1 --> Stage2 --> Stage3 --> End([종료])
```
