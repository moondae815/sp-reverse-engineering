...
| **Milestone 2** | **설계 명세서 신뢰성 검증** | AI가 분석한 최초 설명서 | 문법 검사 및 도표가 포함된 설명서 파일 저장 | 병렬 분석 중 진행 상태를 실시간 스피너와 프로그레스 바(TUI Progress)로 보여주며, AI 안전 차단 등 API 에러 시 세부 사유를 명시해 극복 (**3중 검증 및 실시간 가시성**) |
| **Milestone 3** | **비용 통제 및 캐시 적용** | 재분석 요청 | 변경되지 않은 프로그램은 분석을 건너뛰고 빠른 화면 표시 및 Serilog 기반 비파괴식 실행 로그 파일 누적 | 이미 분석한 내용은 호출하지 않아 AI 사용 요금을 절감하며 실행 로그 보존 (**비용 통제 & 추적성**) |
...
*   **비즈니스 목적**: AI가 부적절한 결과나 린트 오류가 포함된 문서를 만들지 못하게 차단함과 동시에, 병렬 수행 중 대기하는 시간 동안 실시간 진행 상태(TUI Progress)를 가시화해 UX를 개선하고 완성도 높은 기능 명세서를 저장합니다.
*   **상세 진행 단계**:
    1.  **실시간 병렬 진행률 가시성 제공**: `dynamic` 추론 강도가 작동하여 다중 후보군(Low/Medium/High Effort)이 병렬 생성 및 Critic 검토에 돌입할 때, Spectre.Console의 실시간 프로그레스 바(Bar)와 개별 소요 시간을 실시간 애니메이션 스피너와 함께 보여줍니다. (Core 비즈니스와 UI 렌더링 간의 결합도를 낮추는 `IMultiProgressScope` 아키텍처 및 Sandbox 세이프티 적용)
    2.  **API 에러 세부 복구**: API 호출 과정에서 안전 정책(Safety Block, Recitation) 또는 장애로 인해 JSON 응답 내 결과 프로퍼티가 누락되었을 때, 정체불명의 `KeyNotFoundException`으로 크래시가 나지 않도록 `TryGetProperty` 가드를 적용하고 구체적인 API 거절 원인을 사용자에게 노출합니다.
    3.  **1차 기계 린팅 검사**: 각 후보의 문서 헤더 서식 및 Mermaid 다이어그램 구문에 기계적 문법 오류가 없는지 컴퓨터가 검증합니다.
    4.  **2차 AI 상호 채점 및 영역별 점진적 합성**: 설정에 의해 지정된 **Critic** 에이전트가 다차원 기준으로 세 후보의 각 파트를 정량 평가 및 채점한 후, **Consolidator** 에이전트가 각 후보의 가장 우수한 파트 조각들을 가져와 하나의 명세서로 조립하고 자연어 톤을 보완하여 단일 통합 명세서를 합성합니다. (자가 편향 극복을 위해 서로 다른 이종 모델의 앙상블 구성을 권장합니다.)
    5.  **3차 사람 최종 승인**: 최종 합성된 문서를 사용자가 화면에서 실시간으로 미리보고 피드백을 전달해 수정한 뒤, 최종 승인 시 저장 및 데이터베이스 Extended Properties 역반영을 실행합니다.
