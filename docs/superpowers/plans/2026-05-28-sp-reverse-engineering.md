...
**Architecture:** 솔루션을 핵심 비즈니스 로직 및 AI 연동을 담당하는 클래스 라이브러리(`ReSet.Core`), Spectre.Console 기반 TUI를 제공하는 콘솔 애플리케이션(`ReSet.Cli`), 그리고 단위 테스트를 위한 xUnit 프로젝트(`ReSet.Core.Tests`)로 구성합니다.
...
### Task 1: ReSet.Core 프로젝트 구조 및 데이터 모델 셋업

**Files:**
- Create: `src/ReSet.Core/Models/SpDefinition.cs`
- Create: `src/ReSet.Core/Models/DependencyInfo.cs`
- Create: `tests/ReSet.Core.Tests/ModelsTest.cs`

- [x] **Step 1: SpDefinition 및 DependencyInfo 모델 매핑을 검증하는 실패하는 테스트 작성**

파일 생성: `tests/ReSet.Core.Tests/ModelsTest.cs`
```csharp
using Xunit;
...
파일 생성: `src/ReSet.Core/Models/DependencyInfo.cs`
```csharp
...
파일 생성: `src/ReSet.Core/Models/SpDefinition.cs`
```csharp
...
실행:
```bash
git add src/ReSet.Core/Models/DependencyInfo.cs src/ReSet.Core/Models/SpDefinition.cs tests/ReSet.Core.Tests/ModelsTest.cs
...
**Files:**
- Create: `src/ReSet.Core/Services/IDbMetadataService.cs`
- Create: `src/ReSet.Core/Services/DbMetadataService.cs`
- Create: `tests/ReSet.Core.Tests/DbMetadataServiceTests.cs`

- [x] **Step 1: SQL Server 메타데이터 조회를 검증하는 테스트 인터페이스 및 가짜 연결 테스트 작성**

파일 생성: `tests/ReSet.Core.Tests/DbMetadataServiceTests.cs`
```csharp
using System.Collections.Generic;
...
파일 생성: `src/ReSet.Core/Services/IDbMetadataService.cs`
```csharp
using System.Collections.Generic;
using System.Threading.Tasks;
using ReSet.Core.Models;
...
파일 생성: `src/ReSet.Core/Services/DbMetadataService.cs`
```csharp
using System;
using System.Collections.Generic;
using System.Threading.Tasks;
...
실행:
```bash
git add src/ReSet.Core/Services/IDbMetadataService.cs src/ReSet.Core/Services/DbMetadataService.cs tests/ReSet.Core.Tests/DbMetadataServiceTests.cs
...
- Create: `tests/ReSet.Core.Tests/DbMetadataServiceDetailsTests.cs`

- [ ] **Step 1: 세부 정보 조회가 실패하는 경우(잘못된 연결 주소) 예외 검증 테스트 작성**

파일 생성: `tests/ReSet.Core.Tests/DbMetadataServiceDetailsTests.cs`
```csharp
using System;
using System.Threading.Tasks;
using Xunit;
...
파일 수정: `src/ReSet.Core/Services/DbMetadataService.cs`의 `GetSpDetailsAsync` 메서드를 다음과 같이 교체합니다.
```csharp
...
**Files:**
- Create: `src/ReSet.Core/Services/IAiService.cs`
- Create: `src/ReSet.Core/Services/AiService.cs`
- Create: `tests/ReSet.Core.Tests/AiServiceTests.cs`

- [x] **Step 1: AI API 연동(HttpClient 통신) 실패 및 성공 시나리오 테스트 작성**

파일 생성: `tests/ReSet.Core.Tests/AiServiceTests.cs`
```csharp
using System;
using System.Threading.Tasks;
using Xunit;
...
파일 생성: `src/ReSet.Core/Services/IAiService.cs`
```csharp
using System.Threading.Tasks;
using ReSet.Core.Models;
...
파일 생성: `src/ReSet.Core/Services/AiService.cs`
```csharp
using System;
...
**Files:**
- Create: `src/ReSet.Cli/SessionManager.cs`
- Create: `tests/ReSet.Core.Tests/SessionManagerTests.cs`

- [x] **Step 1: 마지막 로그인 아이디를 저장하고 가져오는 기능의 실패/성공 테스트 작성**

파일 생성: `tests/ReSet.Core.Tests/SessionManagerTests.cs`
```csharp
using System.IO;
using Xunit;
using ReSet.Cli;
...
파일 생성: `src/ReSet.Cli/SessionManager.cs`
```csharp
using System.IO;
using System.Text.Json;
...
실행:
```bash
git add src/ReSet.Cli/SessionManager.cs tests/ReSet.Core.Tests/SessionManagerTests.cs
...
**Files:**
- Create: `src/ReSet.Cli/appsettings.json`
- Create: `src/ReSet.Cli/instructions.txt`
- Modify: `src/ReSet.Cli/Program.cs`

- [x] **Step 1: 설정 파일 및 지침 파일 생성**

파일 생성: `src/ReSet.Cli/appsettings.json`
```json
{
  "DatabaseSettings": {
    "Server": "localhost",
...
파일 생성: `src/ReSet.Cli/instructions.txt`
```text
...
파일 생성/수정: `src/ReSet.Cli/Program.cs`
```csharp
using System;
using System.IO;
using System.Threading.Tasks;
...
실행: `dotnet run --project src/ReSet.Cli`
...
실행:
```bash
git add src/ReSet.Cli/appsettings.json src/ReSet.Cli/instructions.txt src/ReSet.Cli/Program.cs
...
파일 수정: `src/ReSet.Cli/Program.cs` 내의 `Main` 함수 뒷부분("데이터베이스 연결 성공!" 문구 아래)을 다음과 같이 수정합니다.
```csharp
            // 3. 서비스 구성
            ...
            while (true)
            {
                ...
                ReSet.Core.Models.SpDefinition spDef = null;
