...
**Architecture:** `ReSet.Cli` 하위에 인자 바인딩 모델 `CliArgs` 및 정적 파서를 추가하여 실행 분기를 제어하고, `instructions.txt` 및 `AiService` 프롬프트를 보강해 Mermaid Flowchart 출력을 필수화합니다.
...
**Files:**
- Create: `src/ReSet.Cli/CliArgs.cs`
- Create: `tests/ReSet.Core.Tests/CliArgsTests.cs`

- [ ] **Step 1: CLI 옵션 배열 파싱 및 환경 변수 연동이 정상 맵핑되는지 검증하는 xUnit 테스트 작성**

파일 생성: `tests/ReSet.Core.Tests/CliArgsTests.cs`
```csharp
using System;
using Xunit;
using ReSet.Cli;
...
파일 생성: `src/ReSet.Cli/CliArgs.cs`
```csharp
...
파일 수정: `src/ReSet.Cli/Program.cs` 클래스 내부에 다음 정적 메서드를 삽입합니다. (Program 클래스 선언부에 `using ReSet.Cli;` 또는 필요한 네임스페이스가 겹치지 않게 조율합니다.)
```csharp
...
```bash
git add src/ReSet.Cli/CliArgs.cs src/ReSet.Cli/Program.cs tests/ReSet.Core.Tests/CliArgsTests.cs
...
파일 수정: `src/ReSet.Cli/Program.cs` 의 `Main` 메서드 본문을 다음과 같이 수정 및 결합하여 대화형/비대화형 분기를 구현합니다.
```csharp
...
**Files:**
- Modify: `src/ReSet.Cli/instructions.txt`
- Modify: `src/ReSet.Core/Services/AiService.cs`

- [ ] **Step 1: instructions.txt 규칙 보완**

파일 수정: `src/ReSet.Cli/instructions.txt` 파일 맨 아래 부분에 아래 내용 추가
```text
...
파일 수정: `src/ReSet.Core/Services/AiService.cs`
...
```bash
git add src/ReSet.Cli/instructions.txt src/ReSet.Core/Services/AiService.cs
