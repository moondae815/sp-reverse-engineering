...
**Architecture:** `IMetadataExporter` 인터페이스를 `ReSet.Core` 아래 정의하고, CLI 환경설정 파일의 플래그 옵션(`SaveRawJson`, `SaveRawContext`, `SaveRawFiles`)에 따라 조건부로 로컬 파일 시스템에 안전하게 저장하도록 `MetadataExporter`를 구현합니다.
...
**Files:**
- Create: `src/ReSet.Core/Services/IMetadataExporter.cs`
- Create: `src/ReSet.Core/Services/MetadataExporter.cs`
- Create: `tests/ReSet.Core.Tests/MetadataExporterTests.cs`

- [x] **Step 1: 임시 디렉토리를 사용해 JSON 파일 덤프를 생성하는지 검증하는 실패하는 xUnit 테스트 작성**

파일 생성: `tests/ReSet.Core.Tests/MetadataExporterTests.cs`
```csharp
using System.IO;
...
파일 생성: `src/ReSet.Core/Services/IMetadataExporter.cs`
```csharp
using System.Threading.Tasks;
using ReSet.Core.Models;
...
파일 생성: `src/ReSet.Core/Services/MetadataExporter.cs`
```csharp
using System.IO;
using System.Text;
using System.Text.Json;
...
파일 수정: `src/ReSet.Cli/Program.cs`
