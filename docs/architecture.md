...
* **L1 (기계 검사 - 정적 Linter)**: 필수 섹션 존재 유무 및 Mermaid 다이어그램 구문(따옴표 규칙 등)을 정규식 기반으로 사전 정적 린팅합니다.
* **L2 (AI 교차 리뷰어)**: 별도의 독립적인 수석 아키텍트 프롬프트 컨텍스트를 가동하여, 생성된 1차 분석서와 원본 DDL을 대조해 비즈니스 로직 왜곡이 없는지 검토하여 누락 발견 시 1회 자가 보완 재생성(`Self-Correction`)합니다.
* **L3 (인간 개입 조율 - HITL)**: 대화형 TUI 모드 시 Spectre.Console 미리보기를 통해 개발자가 승인하거나 직접 자연어 보완 피드백을 주어 최종 문서를 정교하게 조율합니다.

### 4. 현대화 배치 스케줄러 전환 설계 모듈
* 1차 마이그레이션 타겟인 **SQL Server Agent 배치 작업** 특성에 특화된 현대화 문서 작성 엔진입니다.
* Agent Job 스케줄 정보(실행 주기, 트리거)를 분석해 최신 배치 아키텍처(C# .NET Worker Service 등)의 Cron 스케줄로 자동 매핑합니다.
* 대용량 데이터 연산 시 발생하는 OOM을 방지하기 위한 청크(Chunk) 페이징 의사코드 및 데이터 정합성 검증 SQL 세트를 도출합니다.
