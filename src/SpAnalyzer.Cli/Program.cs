            ...
            if (cliArgs.IsBatchMode)
            {
                ...
                foreach (var selectedOption in targetSps)
                {
                    ...
                    try
                    {
                        ...
                        await AnsiConsole.Status()
                            .StartAsync($"[yellow]{selectedOption}[/] 분석 진행 중...", async ctx =>
                            {
                                ...
                                {
                                    throw new Exception("SP 메타데이터 수집 실패");
                                }
                            });

                        if (!processSuccess || string.IsNullOrEmpty(specificationMarkdown))
                        {
                            throw new Exception("리버스 엔지니어링 명세서 생성 실패");
                        }
...
";
                        ...
                        AnsiConsole.MarkupLine($"[green]성공:[/] {selectedOption} 분석 완료 -> {outputFileName}");
                    }
                    catch (Exception ex)
                    {
