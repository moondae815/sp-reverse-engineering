...
1. **지식/환경 관측 및 수집 (Perception & Knowledge Gathering)**
   * 타겟 SP가 참조하는 외부 테이블의 컬럼 구성, 기본키(PK)/외래키(FK) 등 관계형 DB의 복잡한 스키마 메타데이터를 직접 질의하여 파악합니다.
...
3. **도구 및 언어 모델 협업 (Tool Use & LLM Collaboration)**
   * 사전에 정의된 지침서(`instructions.txt`)와 획득한 기계적 지식 데이터를 바탕으로 AI 전용 구조화 프롬프트를 자율 조립합니다.
   * 언어 모델에게 명확한 다이어그램 생성 문법을 요청하고 가공된 최종 산출물을 마크다운 포맷으로 전달받아 로컬 시스템에 영속화합니다.
...
* **유연한 다중 모드 지원 (Dual Execution Modes)**
  * **대화형 TUI 모드**: Spectre.Console 기반의 미려한 로그인 프롬프트, 실시간 자동완성 검색, 스피너 로딩 UI를 제공합니다.
  * **배치 및 CLI 자동화 모드**: 명령줄 아규먼트와 환경 변수를 활용해 TUI 화면을 건너뛰고 스케줄러 등을 통한 대량 무인 처리를 지원합니다.
* **오류 격리 및 소프트 페일 (Soft Fail / Fault Tolerance)**
  * 메타데이터를 파일로 내보내거나 대량 일괄 분석을 수행하는 중 특정 SP 혹은 AI API 호출에서 장애가 발생해도, 해당 단계만 경고 로깅 후 스킵하여 전체 워크플로우가 멈추지 않는 격리 아키텍처를 가집니다.
* **시각화 다이어그램 포함 명세화 (Visual Sequence Generation)**
  * 마크다운으로 결과물을 도출할 때 비즈니스 제어 흐름과 데이터의 CRUD 흐름을 파악하기 좋도록 Mermaid Flowchart 생성을 함께 수행합니다.

---

## 📊 프로그램 실행 흐름 (Visual Execution Flow)

아래 다이어그램은 SP Analyzer 프로그램이 기동되어 설정 파싱, 데이터베이스 메타데이터 재귀 수집, AI 분석 및 결과물 영속화까지 거쳐 가는 전체 자율 실행 흐름을 시각적으로 나타냅니다.
