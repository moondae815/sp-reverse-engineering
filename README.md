...
* **T-SQL AST 정적 분석 (ScriptDom)**: Microsoft 공식 ScriptDom 분석기를 탑재하여 DDL의 CRUD 성격별(SELECT/INSERT/UPDATE/DELETE) 테이블 분류, 중첩 분기(IF/WHILE) 들여쓰기 요약, 동적 SQL 및 UDF/Linked Server 원격 참조 자동 감지, 그리고 데이터베이스 호환성 수준별 파서 동적 매핑을 지원합니다.
