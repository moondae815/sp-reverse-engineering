...
- **하이브리드 분석**: SQL Server 시스템 뷰(`sys.sql_expression_dependencies` 등)를 조회하여 정확한 DB 의존성 목록을 수집하고, SP 본문 DDL 코드와 함께 AI에 제공하여 분석 정확도를 극대화합니다.
...
- **테스트 주도 개발(TDD) 기반**: 솔루션 코드가 5개의 xUnit 단위 테스트 코드를 통해 견고하게 검증되어 있습니다.
