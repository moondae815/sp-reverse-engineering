...
* **T-SQL AST 정적 분석 고도화 (ScriptDom)**: Microsoft 공식 TransactSql.ScriptDom 분석기를 탑재하여 DDL DDL 소스 코드를 기계적으로 파싱합니다.
  * **CRUD 분류 매핑**: 참조하는 테이블을 단순 수집하는 데 그치지 않고, SELECT/INSERT/UPDATE/DELETE 구문 성격별로 분류해 맵핑합니다.
  * **중첩 제어 흐름 들여쓰기**: IF/WHILE 분기가 중첩된 구조에서 깊이(depth)에 맞춤 들여쓰기(2칸씩 공백 추가)를 적용해 가독성 있는 조건 계층을 요약합니다.
  * **동적 SQL 감지 경고**: sp_executesql 및 EXEC (@sql) 구문을 감지해 분석 한계 경고를 명세 및 AI 가이드에 마킹합니다.
  * **호환성 레벨 파서 팩토리**: DB 호환성 수준(100~160)을 실시간으로 쿼리하여 그에 매핑되는 파서(SQL 2012 ~ 2022)를 동적으로 기동해 구문 오류를 원천 차단합니다.
  * **UDF 및 Linked Server 감지**: 스키마가 수반된 UDF 함수(dbo.fn_CalculateTax) 및 4파트 식별자 형태의 Linked Server 테이블 참조(MyServer.RemoteDb.dbo.Orders)를 감지하고 경고를 주입하여 마이그레이션 전략 수립을 돕습니다.
