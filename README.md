...
- **배치 현대화 설계(Batch Modernization Planner) 지원**:
  - SQL Server Agent 스케줄러로 작동하는 배치성 SP들을 현대적인 백그라운드 배치/스케줄러 시스템으로 성공적으로 이행하기 위한 `[Schema].[SP이름]_BatchMigrationPlan.md` 전환 계획 설계서 자동 생성 기능을 지원합니다.
  - **신규 배치 프레임워크 설계**(C# .NET Worker Service, Java Spring Batch 등 지정된 스택 매핑), **대용량 데이터 청크(Chunk) 페이징 연산**, **로깅/실패 알림(Slack 등) 통합 계획**, 그리고 **데이터 정합성 검증 SQL 세트**를 도출해 줍니다.
...
- **테스트 주도 개발(TDD) 기반**: 솔루션 코드가 최종 추가된 검증 케이스를 포함한 17개의 xUnit 단위 테스트 코드를 통해 견고하게 검증되어 있습니다.
...
3. 로딩이 완료되면 지정된 출력 디렉터리(기본 `./output`) 내부에 `[스키마].[SP이름]_Spec.md` 분석 명세서와 함께 스케줄러 배치 마이그레이션용 `[스키마].[SP이름]_BatchMigrationPlan.md` 전환 계획서가 생성됩니다.
